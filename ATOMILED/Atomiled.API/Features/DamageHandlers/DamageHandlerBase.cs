        ...
        protected DamageType GetDamageType(BaseHandler damageHandler = null)
        {
            ...
            switch (damageHandler)
            {
                ...
                case UniversalDamageHandler universal:
                    ...
                    Log.Warn($"{nameof(DamageHandler)}.{nameof(Type)}: No matching {nameof(DamageType)} for {nameof(UniversalDamageHandler)} with ID {translation.Id}, type will be reported as {DamageType.Unknown}. Report this to ATOMILED Devs.");
                    break;
