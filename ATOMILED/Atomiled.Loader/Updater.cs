...
#pragma warning disable SA1310 // Field names should not contain underscore
    ...
    internal sealed class Updater
    {
        ...
        private IEnumerable<AtomiledLib> AtomiledLib =>
            from Assembly a in AppDomain.CurrentDomain.GetAssemblies()
            let name = a.GetName().Name
            where name.StartsWith("Atomiled.", StringComparison.OrdinalIgnoreCase) &&
            !(config.ExcludeAssemblies?.Contains(name, StringComparison.OrdinalIgnoreCase) ?? false) &&
            name != Assembly.GetExecutingAssembly().GetName().Name
            select new AtomiledLib(a);

        private string Folder => File.Exists(Path.Combine(LabApi.Loader.Features.Paths.PathManager.Plugins.FullName, "global", "Atomiled.Loader.dll")) ? "global" : Server.Port.ToString();
        ...
        internal void CheckUpdate()
        {
            try
            {
                ...
                if (Busy = FindUpdate(client, !LabApi.Loader.PluginLoader.Dependencies.Any(x => x.GetName().Name == "Atomiled.API"), out NewVersion newVersion))
        ...
        private HttpClient CreateHttpClient()
        {
            ...
            client.DefaultRequestHeaders.Add("User-Agent", $"Atomiled.Loader (https://github.com/Exmod-Team/ATOMILED, {Assembly.GetExecutingAssembly().GetName().Version.ToString(3)})");
        ...
        private bool FindUpdate(HttpClient client, bool forced, out NewVersion newVersion)
        {
            try
            {
                ...
                AtomiledLib smallestVersion = AtomiledLib.Min();

                Log.Info($"Found the smallest version of Atomiled - {smallestVersion.Library.GetName().Name}:{smallestVersion.Version}");
                ...
                {
                    // No errors
                    Log.Info("No new versions found, you're using the most recent version of Atomiled!");
                }
            }
            catch (Utf8Json.JsonParsingException)
            {
                Log.Warn("Encountered GitHub ratelimit, unable to check and download the latest version of Atomiled.");
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Updates the client's version of Atomiled.
        /// </summary>
        /// <param name="client"> is the HTTP Client.</param>
        /// <param name="newVersion"> is the updated version of Atomiled.</param>
        private void Update(HttpClient client, NewVersion newVersion)
        {
            try
            {
                ...
                if (installerProcess.ExitCode == 0)
                {
                    ...
                    ServerLogs.AddLog(ServerLogs.Modules.Administrative, "ATOMILED scheduled server restart after the round end.", ServerLogs.ServerLogType.RemoteAdminActivity_GameChanging, false);
        ...
        private bool FindRelease(TaggedRelease[] releases, out Release release, AtomiledLib smallestVersion, bool forced = false)
        {
            bool includePRE = config.ShouldDownloadTestingReleases || AtomiledLib.Any(l => l.Version.PreRelease is not null);
