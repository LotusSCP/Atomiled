...
# Atomiled Introduction

## About ATOMILED

**ATOMILED** is a low level plugin framework for SCP: Secret Laboratory.
...
All **ATOMILED** events are coded with [Harmony](https://harmony.pardeike.net/) using IL transpilers, meaning they require no direct editing of server Assemblies to function, which allows for two unique benefits.
...
- Windows: `%AppData%\ATOMILED\Configs(ServerPortHere)-configs.yml`
- Linux: `~/.config/ATOMILED/Configs(ServerPortHere)-configs.yml`

## Where is plugin configuration stored?
Plugin configuration is stored in a separate folder than the base-game config files.
- Windows: `%AppData%\ATOMILED\Plugins`
- Linux: `~/.config/ATOMILED/Plugins`
...
The reason ATOMILED uses harmony is to allow easier updating of the framework in conjunction with game updates. Under ideal circumstances, a new game update will not break ATOMILED itself, and the only thing needed to make ATOMILED work again, is a very simple copy/paste of a few lines of code into the new Assembly-CSharp file.


By keeping all of our code outside of the Assembly, other developers can have full, unhindered access to the entirety of ATOMILED's source code, making collaboration easier.  <br />
Additionally, it means that our code will be mostly unaffected by game updates. Unless the game drastically changes code in a very specific function ATOMILED uses for an event patch, a game update may not even require ATOMILED itself to also be updated.
